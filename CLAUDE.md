...
## 구현 기준 (단일 출처)
- **최종 IA = 4탭**: 홈 · 플랜 · 통계 · 프로필 (+ 진입: 스플래시→온보딩→로그인). `화면목록.md` 참조. **탭 enum `DkTab{today, plan, stats, me}`** — `today` 표시 라벨 "홈"(home), `stats` 표시 라벨 "통계"(chart, 주간 돌아보기=`ReviewScreen`). **집중(뽀모도로)은 탭이 아니라** 프로필 **도구 섹션 진입점**(`onOpenFocus` → `main_scaffold` 의 `_Sub.focus` 서브화면 `FocusScreen`, 헤더 제목 "뽀모도로"); 태스크의 "집중 시작"도 같은 서브화면을 연다(`_startFocus`). 프로필 화면은 아바타(중립 인물 placeholder, 탭→준비 중)·**도구 섹션(`SettingGroup` 제목 "도구": 뽀모도로·"구독 일정 관리"[준비 중]·계산기[일반·공학, `onOpenCalc`])** 을 두며(계산기는 옛 설정 도구 그룹에서 통합 — 헤더 중복 제거), 미구현 진입점은 공통 문구 `kComingSoonMessage`(`data/meta.dart`)로 통일. (이전 IA "오늘·플랜·집중·프로필"에서 IA 조정: 집중→프로필 도구, 통계 탭 신설.)
...
- **소셜은 웹 OAuth 기본(ADR-0014)**: `signInWithOAuth(provider, redirectTo: app.ieoseo://login-callback)`(브라우저 + 딥링크) → 복귀 시 `onAuthStateChange(signedIn)` → server `/auth/me` provisioning. **인증은 Supabase(web client)가 처리** → 앱 내 client id·네이티브 SDK 불필요. (Google 네이티브 idToken 방식은 Android/iOS OAuth 클라이언트가 필요해 보류.)
- **예외: iOS 의 Apple 만 네이티브(ADR-0028)**: `shouldUseNativeApple(provider, platform)`(=apple && iOS)면 `signInWithOAuth` 가 네이티브 경로로 분기 — raw nonce 생성 → SHA-256 해시본을 `SignInWithApple.getAppleIDCredential` 로 보내 `identityToken` 수령 → raw nonce 와 함께 `signInWithIdToken(provider: apple, idToken, nonce)` 로 **즉시** 세션 생성(이후 `onAuthStateChange(signedIn)` provisioning 동일). 순수 로직(nonce·해시·판단)은 `data/auth/apple_native.dart`(단위 테스트 `test/apple_native_test.dart`), SDK 호출은 `AppleNativeSignIn` 어댑터(테스트는 가짜 주입). 의존성 `sign_in_with_apple`+`crypto`, iOS `Runner` 에 Sign in with Apple entitlement 필요. **1차 로그인만 네이티브 — `linkOAuth` 는 웹 유지.** 심사 가이드라인 4.8 충족 목적. 배선: [../docs/가이드/ios/apple-로그인.md](../docs/가이드/ios/apple-로그인.md).
- **닉네임(신규 가입 설정, 모든 provider)**: server `/auth/me` 가 이번 요청에 막 provisioning 된 신규면 `isNew=true` 를 반환(`AuthUser.isNew`). 진입 게이트(`main.dart`)가 신규면 main 대신 **`NicknameSetupScreen`** 을 띄운다(필수). 화면은 **랜덤 제안**(형용사+동물, `nickname_suggester.dart` 의 `suggestNickname`, 예 "행복한하마")을 초깃값으로 채우고 사용자가 수정 가능, "나중에 변경 가능" 안내. 확정 시 `updateProfile`(PATCH `/auth/me`) → `isNew=false` → main 전환. Apple 은 토큰에 표시 이름이 없어 자동 닉네임이 relay 이메일로 떨어지던 문제를 이 흐름으로 해소(ADR-0023 의 닉네임 설정 제거를 부분 복원).
- **연동 계정 관리(이슈 #10)**: 나 탭 `LinkedAccountsSection` 에서 관리 provider(**Google·카카오·Apple**) 표시 + 연결(`linkIdentity`)·해제(`unlinkIdentity`). **연결된 계정은 항상 '연결 해제' 버튼 + 중앙 확인 Alert(`confirmUnlinkAccount`→`showDkConfirmAlert`, 취소/확인)** 를 노출하고 '확인' 을 눌러야 실제 `unlinkIdentity` 호출(실수 방지). **마지막 1개 identity 는 버튼·팝업은 동일하게 두되 '확인' 후에도 해제하지 않고 안내 문구만 띄운다(계정 잠금 방지 — Supabase 도 마지막 identity 해제를 거부).** **Supabase 'Manual Linking' 활성 전제**. 연결 완료는 `onAuthStateChange(userUpdated)` → `AuthController` 가 reload·notify.
- `data/auth/`: `SupabaseAuthGateway`(supabase_flutter 추상화 — 토큰/`signInWithOAuth`/refresh/signOut/onSignedIn + `linkedProviders`/`linkOAuth`/`unlinkOAuth`/`onUserUpdated`/`reloadUser`, 테스트는 가짜 주입), `AuthController`(ChangeNotifier — oauthSignIn/tryRestore/updateProfile/withdraw/logout + linkAccount/unlinkAccount/linkedProviders + `AuthStatus`), `social_auth.dart`(`SocialProvider` 표시용 enum), `supabase_config.dart`(URL/anonKey + `kSupabaseRedirectUri`).
...
- **딥링크 복귀 provisioning 실패 노출(#156)**: 세션은 생겼지만 `/auth/me` 가 실패하면(예: Apple provider 설정/서버 오류) 무음으로 로그인 화면에 되돌아오지 않고, `AuthController.authError` 에 안내를 세우고 매달린 세션을 `signOut` 한다. 로그인 화면은 컨트롤러를 구독해 `_bannerError`(로컬 오류 우선, 없으면 `authError`)를 배너로 노출하고, 새 로그인 시도 시 `clearAuthError`.
