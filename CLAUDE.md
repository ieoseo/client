...
- **연동 계정 관리(이슈 #10)**: 나 탭 `LinkedAccountsSection` 에서 관리 provider(**Google·카카오·Apple**) 표시 + 연결(`linkIdentity`)·해제(`unlinkIdentity`). 마지막 identity 는 해제 불가. 해제는 **확인 시트(`confirmUnlinkAccount`)** 통과 후 실제 `unlinkIdentity` 호출(실수 방지). **Supabase 'Manual Linking' 활성 전제**. 연결 완료는 `onAuthStateChange(userUpdated)` → `AuthController` 가 reload·notify.
